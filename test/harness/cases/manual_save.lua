return function(H)
  ...
  ok(H.update_registered("VermilionLibPulse"), "the library icon pulses when a session lands")
  H.advance(800)
  ok(not H.update_registered("VermilionLibPulse"), "the pulse ends on its own")
  ok((VermilionGraphWindowLibBtn._alpha or 1) == 1, "the library icon comes back to full alpha")
  ...
  Vermilion.Library.show()
  ok(VermilionLibraryListEmpty._hidden == true, "the library lists the sessions")
  ok(VermilionLibraryRow1When._text:find("edit_save", 1, true) == nil, "the autosaved row carries no hand glyph")
  ok(VermilionLibraryRow2When._text:find("edit_save", 1, true) ~= nil, "the manual row carries the hand glyph")
  ok(VermilionLibraryRow2Sel._hidden == false, "opening the library lands on the manual session even after a later autosave")
  ok(VermilionLibraryRow1Sel._hidden == true, "an autosave never pre-selects a row")
  ...
  Vermilion.Library.show()
  ok(VermilionLibraryRow1Sel._hidden == true, "an autosave with the library closed pre-selects nothing")
  Vermilion.Library.on_row_click(2)
  H.state.zone = "Spindleclutch"
  sv.settings.session_autosave = false
  Vermilion.Graph.on_flush_click()
  Vermilion.Graph.on_record_click()
  H.damage_out({ hit = 650 })
  H.advance(1000)
  Vermilion.Graph.on_stop_click()
  ok(Vermilion.Graph.on_save_click() == true, "manual save with the library open")
  H.advance(400)
  ok(SS.count() == 4, "four sessions")
  ok(VermilionLibraryRow1Name._text == "Spindleclutch", "the open library refreshes with the new session first")
  ok(VermilionLibraryRow1Sel._hidden == false, "and selects it, label box ready")
  Vermilion.Library.hide()
  Vermilion.Graph.toggle_record()
