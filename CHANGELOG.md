...
- The session library draws a heat strip under each session's numbers: the fight's output compressed into 24 cells on the pressure ramp, so a fight is recognisable at a glance. Sessions saved before this version show no strip.
