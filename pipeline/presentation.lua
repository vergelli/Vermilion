...
-- The single stable payload instance, reused every tick. eos_groups is a
-- pre-allocated array; eos_groups.count holds the number of live entries.
local payload = {
  ts         = 0,
  eDPS       = 0,
  ShDPS      = 0,
...
-- Builds the snapshot for now_ms. Returns the same payload table every call;
-- consumers must read fields (or copy) before the next snapshot.
